...
func main() {
	...
	created_text := buildString(text_words)
	print(created_text)
}
...
func buildString(text_words []string) string {
	text_buffer := bytes.NewBufferString("")
	...
	for i, word := range text_words {
		...
		if word_end == '?' || word_end == '!' || word_end == '.' {
			...
			is_uppercase_next_word_first_letter = false
			uppercased_str := strings.ToUpper(word[0:1]) + word[1:]
